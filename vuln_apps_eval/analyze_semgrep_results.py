...
def load_semgrep_results(semgrep_file):
    """Load and parse Semgrep JSON results."""
    ...
    for result in data.get('results', []):
        results.append({
            'vulnerability': result.get('extra', {}).get('message', 'Unknown'),
...
def paths_match(semgrep_path, gt_path):
    semgrep_normalized = normalize_path(semgrep_path)
    gt_normalized = normalize_path(gt_path)
    
    return (semgrep_normalized.endswith(gt_normalized) or 
            gt_normalized.endswith(semgrep_normalized) or
            semgrep_normalized == gt_normalized)

def line_in_range(line, start, end):
    return start <= line <= end

def analyze_vulnerabilities(semgrep_data, ground_truth):
            ...
            if paths_match(file_path, gt_file):
                for vuln_line, challenges in lines_dict.items():
                    ...
                    for challenge_info in challenges:
                        ...
                        # sink is within ground truth range
                        elif line_in_range(end_line, gt_start, gt_end):
                            matched = True
...
def print_results(tp, fp, fn, metrics):
    ...
    print("\n" + "="*80)
    print("SEMGREP VULNERABILITY ANALYSIS RESULTS")
    print("="*80)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python analyze_semgrep_results.py <semgrep_results.json> <ground_truth.json> [output.json]")
        sys.exit(1)
    ...
    print(f"Loading Semgrep results from: {semgrep_file}")
    semgrep_data = load_semgrep_results(semgrep_file)
    print(f"Loaded {len(semgrep_data)} detections")
    ...
    tp, fp, fn = analyze_vulnerabilities(semgrep_data, ground_truth)
