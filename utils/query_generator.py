...
def generate_conditional_sanitizer_library(classified_methods, output_path, track_query_fn=None, validation_db: str | None = None):
    ...
    # Post-process: merge any duplicate helper predicates the LLM emitted
    # independently for different sanitizers (e.g. hasNorOperator defined
    # once per mongoose method).  OR-merge preserves all cases.
    with open(output_path, 'r') as f:
        qll_content = f.read()
    deduped = deduplicate_helper_predicates(qll_content)
    if deduped != qll_content:
        with open(output_path, 'w') as f:
            f.write(deduped)
        logger.info(f"[generate_conditional_sanitizer_library] Duplicate helper predicates merged in {output_path}")

    logger.info(f"Successfully generated conditional sanitizer library at {output_path}")
    return output_path

def deduplicate_helper_predicates(code: str) -> str:
    """
    Merge duplicate helper predicate definitions into a single predicate
    whose body is the union (``or``) of all variant bodies.

    WHY OR-MERGE (not keep-first / keep-last)
    ------------------------------------------
    Each duplicate was generated by an independent LLM call approaching the
    same bypass condition.  The variants are complementary subsets of the
    same truth — e.g. one handles object-property recursion, another handles
    array-element recursion.  OR-merging preserves every case from every
    variant, so the result is strictly at least as complete as the best
    individual definition and usually more complete.  Keep-first / keep-last
    silently discards cases.

    SCOPE
    -----
    Only helper predicates are affected — those that are not the explicitly
    named top-level predicate the pipeline requested (isSink, isSource,
    isBarrier, isAdditionalFlowStep, isCWE_*, etc.).  The top-level predicate
    is always unique by construction (the pipeline names it explicitly), so
    it is never touched.

    ALGORITHM
    ---------
    1. Find every predicate header via regex.
    2. Group by normalised header (name + arity).
    3. Single definition → emit unchanged.
    4. Multiple definitions → extract each body (between { and matching }),
       join with ``or``, emit one merged block.
    5. Log a WARNING for every merge so the operator can see it.
    """
    from collections import OrderedDict

    # Matches any predicate header line, capturing indentation + keyword
    # prefix, predicate name, and the parameter list + opening brace.
    HEADER_RE = re.compile(
        r'^([ \t]*(?:private\s+)?predicate\s+)(\w+)(\s*\([^)]*\)\s*\{)',
        re.MULTILINE,
    )
    ...
    segments = []  # (full_header_str, predicate_name, body_str)
    ...
    # differences don't create spurious distinct groups).
    groups: dict = OrderedDict()
    for header, name, body in segments:
        key = re.sub(r'^[ \t]+', '', header, flags=re.MULTILINE)
        groups.setdefault(key, []).append((header, name, body))

    result_parts = [preamble]

    for key, defs in groups.items():
        if len(defs) == 1:
            header, _, body = defs[0]
            result_parts.append(header + body)
            continue

        # --- merge path ---
        first_header, pred_name, _ = defs[0]
        indent_len = len(first_header) - len(first_header.lstrip())
        ind = " " * indent_len

        logger.warning(
            f"[deduplicate_helper_predicates] '{pred_name}': merging "
            f"{len(defs)} duplicate definitions with 'or' — "
            f"all {len(defs)} variants' cases are preserved."
        )

        # Extract the inner body of each definition (strip closing brace).
        inner_bodies = []
        for _, _, body in defs:
            stripped = body.rstrip()
            # Drop the closing "}" that ends this predicate block
            if stripped.endswith("}"):
                inner = stripped[:-1]
            else:
                inner = stripped
            inner = inner.strip("\n")
            inner_bodies.append(inner)

        merged_inner = f"\n{ind}  or\n".join(inner_bodies)
        merged = f"{first_header}\n{merged_inner}\n{ind}}}\n"
...
def general_vuln_query(cwe_id, sink_predicate, sanitizer_predicate, flow_predicate):
    ...
    */
    ...
    # The sink, sanitizer, and flow predicates are generated by independent
    # LLM calls and may each emit the same helper predicate name (e.g.
    # thenCall, sink, concurrentAccess).  Deduplicate before returning so
